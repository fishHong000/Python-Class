...
# 找出n皇后問題有幾解
def findSol(n):
    return mySol(n,0,[True for i in range(n)],[True for i in range(2*n-1)],[True for i in range(2*n-1)])
...
def mySol(n,r,Vok,Uok,Dok):
    ...
    for c in range(n): # r這個row上面的每個column c都試試看
        if Vok[c] and Uok[r+c] and Dok[r-c+n-1]: # 直,上斜,右下斜都可以放
            ...
            numSol = numSol + mySol(n,r+1,Vok,Uok,Dok) # 把可能的解都加總起來 
            Vok[c],Uok[r+c],Dok[r-c+n-1] = True,True,True # 回復原資料
    return numSol

def main():
    print(findSol(int(input())))
